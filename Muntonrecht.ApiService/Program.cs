...
builder.Services.AddDbContext<MuntonrechtContext>(opts => opts.UseNpgsql(builder.Configuration.GetConnectionString("DefaultConnection")));

// Add Identity
builder.Services.AddIdentity<UserModel, IdentityRole>()
    .AddEntityFrameworkStores<MuntonrechtContext>()
...
builder.Services.AddAutoMapper(opts => opts.AddProfile(typeof(MuntonrechtProfile)));
...
{
    var db = scope.ServiceProvider.GetRequiredService<MuntonrechtContext>();
    var migrationLogger = scope.ServiceProvider.GetRequiredService<ILogger<MuntonrechtContext>>();
    SqlScriptGenerator.Generate(); // generate SQL scripts for all entities with [GenerateCrud]
    db.RunMigrations(migrationLogger); // apply auto_*.sql + manual_*.sql, logging each script
}
