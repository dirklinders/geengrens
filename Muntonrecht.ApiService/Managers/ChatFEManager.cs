...
public class ChatFEManager
{
    ...
    private readonly MuntonrechtContext _muntonrechtContext;

    public ChatFEManager(ChatClient chatClient, MuntonrechtContext muntonrechtContext)
    {
        _chatClient = chatClient;
        _muntonrechtContext = muntonrechtContext;
    }
    ...
    {
        var chats = _muntonrechtContext.Chats
    ...
    public async Task<bool> RequestEndAsync(int characterId, int teamId)
    {
        ...
        await _muntonrechtContext.SaveChangesAsync();
        return true;
    }
    ...
    {
        var character = await _muntonrechtContext.Characters.FindAsync([characterId], cancellationToken);
        if (character == null)
        ...
        if (teamId.HasValue)
        {
            var team = await _muntonrechtContext.Teams.FindAsync([teamId.Value], cancellationToken);
        ...
        bool closingRequested = teamId.HasValue && _muntonrechtContext.Chats
            .Any(x => x.CharacterId == characterId && x.TeamId == teamId
                      && x.Role == "System" && x.Message == SluitAfMarker);
        ...
        var previousChats = _muntonrechtContext.Chats
        ...
        {
            _muntonrechtContext.Chats.Add(new ChatModel { CharacterId = characterId, TeamId = teamId, Role = "User",      Message = userInput });
            _muntonrechtContext.Chats.Add(new ChatModel { CharacterId = characterId, TeamId = teamId, Role = "Assistant", Message = assistantReply });
            await _muntonrechtContext.SaveChangesAsync(CancellationToken.None);
        }

        // ── Admin-requested close: lock the conversation and skip Phase 2 ─────────
        if (closingRequested && !string.IsNullOrEmpty(assistantReply))
        {
            _muntonrechtContext.Chats.Add(new ChatModel { CharacterId = characterId, TeamId = teamId, Role = "System", Message = EndedMarker });
            await _muntonrechtContext.SaveChangesAsync(CancellationToken.None);
        ...
        // ── Phase 3: weapon discovery check ──────────────────────────────────
        // If the character has a weapon stop keyword and the team hasn't discovered
        // this weapon yet, check if the keyword appears in the assistant's reply.
        if (teamId.HasValue && !string.IsNullOrWhiteSpace(character.StopKeywordWeapon))
        {
            var weaponKeyword = character.StopKeywordWeapon.ToLowerInvariant();
            var assistantText = previousChats
            ...
            if (assistantText.Contains(weaponKeyword))
            {
                // Check if team already discovered this weapon
                var characterWeapons = await _muntonrechtContext.Weapons
                    .Where(w => w.StopKeywordWeapon == character.StopKeywordWeapon)
                    .ToListAsync(cancellationToken);
    
                foreach (var weapon in characterWeapons)
                {
                    bool alreadyDiscovered = await _muntonrechtContext.TeamWeapons
                        .AnyAsync(tw => tw.TeamId == teamId.Value && tw.WeaponId == weapon.Id, cancellationToken);
    
                    if (!alreadyDiscovered)
                    {
                        _muntonrechtContext.TeamWeapons.Add(new TeamWeaponModel
                        {
                            TeamId = teamId.Value,
                            WeaponId = weapon.Id,
